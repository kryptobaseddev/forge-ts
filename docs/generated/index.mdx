---
title: forge-ts
---

**forge-ts** is a TypeScript documentation toolkit that performs a single AST traversal of your project and produces API docs, OpenAPI specs, executable doctests, and AI context files in one pass.

## Features

- 7 packages with full TypeScript support
- Auto-generated API reference from source code
- Executable `@example` blocks as doctests
- AI-ready context files from a single build pass

## Installation

```bash
npm install -D forge-ts
```

## Quick Example

```typescript
import { getCurrentUser } from "@forge-ts/core/audit";
const user = getCurrentUser(); // e.g. "alice"
```

## Packages

| Package                                | Description             |
| -------------------------------------- | ----------------------- |
| [`core`](/packages/core/index)         | 64 exported symbol(s).  |
| [`.`](/packages/./index)               | 3 exported symbol(s).   |
| [`api`](/packages/api/index)           | 15 exported symbol(s).  |
| [`cli`](/packages/cli/index)           | 108 exported symbol(s). |
| [`gen`](/packages/gen/index)           | 100 exported symbol(s). |
| [`enforcer`](/packages/enforcer/index) | 11 exported symbol(s).  |
| [`doctest`](/packages/doctest/index)   | 16 exported symbol(s).  |

## Next Steps

- [Getting Started](/getting-started) — Step-by-step guide
- [API Reference](/packages) — Full API documentation
- [Concepts](/concepts) — How it works
