 ...
 * @internal
 */
function buildInfo(config: ForgeConfig): OpenAPIInfoObject {
	return {
		title: config.project?.packageName ?? "API Reference",
		version: config.project?.version ?? "0.0.0",
		description: config.project?.description ?? "Generated by forge-ts",
	};
}
