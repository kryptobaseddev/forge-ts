...
| [`runDocsDev()`](./api-reference.md#rundocsdev) | function | Starts the local dev server for the configured SSG target.  Reads `gen.ssgTarget` from the forge-ts config, resolves the adapter, and spawns the platform's dev server in the output directory. |
| [`docsDevCommand`](./api-reference.md#docsdevcommand) | variable | Citty command definition for `forge-ts docs dev`. |
