...
| `"require-fresh-params"`           | `RuleSeverity` | Yes      | W014: `\@param` name in TSDoc does not match actual parameter name in signature.                                           |
| `"require-param-count"`            | `RuleSeverity` | Yes      | W015: `\@param` count in TSDoc does not match actual parameter count in signature.                                         |
| `"require-fresh-returns"`          | `RuleSeverity` | Yes      | W016: `\@returns` tag present on a void/Promisevoid function.                                                              |
| `"require-meaningful-remarks"`     | `RuleSeverity` | Yes      | W017: `\@remarks` block is empty or contains only placeholder text.                                                        |
| `"require-operation-completeness"` | `RuleSeverity` | Yes      | W018: `\@operation`-tagged function missing required CKM documentation (`\@param`, `\@returns`, `\@remarks`, `\@example`). |
| `"require-ckm-tag-content"`        | `RuleSeverity` | Yes      | W019: CKM tag (`\@operation`, `\@constraint`, `\@workflow`, `\@concept`) has empty or insufficient content.                |
| `"require-constraint-throws"`      | `RuleSeverity` | Yes      | W020: `\@constraint`-tagged symbol missing `\@throws` to document constraint violation error.                              |
...
| `tsconfig`        | `string`                                                                                                                                                                                                                                             | Yes      | Path to the `tsconfig.json` used for TypeScript compilation and type resolution.         |
| `outDir`          | `string`                                                                                                                                                                                                                                             | Yes      | Output directory for all generated documentation artifacts.                              |
| `enforce`         | `{ enabled: boolean; minVisibility: Visibility \| "public" \| "beta" \| "internal" \| "private"; strict: boolean; rules: EnforceRules; ignoreFile?: string; }`                                                                                       | No       | Enforcement configuration — controls which TSDoc rules run and at what severity.         |
| `doctest`         | `{ enabled: boolean; cacheDir: string; }`                                                                                                                                                                                                            | Yes      | DocTest configuration — controls execution of `@example` blocks as live tests.           |
| `api`             | `{ enabled: boolean; openapi: boolean; openapiPath: string; }`                                                                                                                                                                                       | Yes      | API generation configuration — controls OpenAPI spec output.                             |
| `gen`             | `{ enabled: boolean; formats: Array<"markdown" \| "mdx">; llmsTxt: boolean; readmeSync: boolean; ssgTarget?: "docusaurus" \| "mintlify" \| "nextra" \| "vitepress" \| "fumadocs"; ckm?: boolean; }`                                                  | No       | Documentation generation configuration — controls what files are written by `forge gen`. |
| `skill`           | `{ enabled?: boolean; customSections?: Array<{ heading: string; content: string; }>; extraGotchas?: string[]; }`                                                                                                                                     | No       | SKILL.md generation settings.                                                            |
| `tsdoc`           | `{ writeConfig: boolean; customTags: Array<{ tagName: string; syntaxKind: "block" \| "inline" \| "modifier"; }>; enforce: { core: "error" \| "warn" \| "off"; extended: "error" \| "warn" \| "off"; discretionary: "error" \| "warn" \| "off"; }; }` | Yes      | TSDoc ecosystem configuration — tag definitions and group-level enforcement.             |
| `bypass`          | `{ dailyBudget: number; durationHours: number; }`                                                                                                                                                                                                    | Yes      | Bypass budget — controls how many temporary rule suppressions are allowed.               |
| `guides`          | `{ enabled: boolean; autoDiscover: boolean; custom: Array<{ slug: string; title: string; sources: string[]; }>; }`                                                                                                                                   | Yes      | Guide generation configuration — controls intelligent guide page output.                 |
| `guards`          | `{ tsconfig: { enabled: boolean; requiredFlags: string[]; }; biome: { enabled: boolean; lockedRules: string[]; }; packageJson: { enabled: boolean; minNodeVersion: string; requiredFields: string[]; }; }`                                           | Yes      | Downstream config drift guards — validate tooling config files stay in sync.             |
| `_configWarnings` | `string[] \| undefined`                                                                                                                                                                                                                              | No       | Warnings generated during config loading (e.g., unknown keys, failed imports).           |
| `project`         | `{ repository?: string; homepage?: string; packageName?: string; description?: string; version?: string; bin?: Record<string, string>; scripts?: Record<string, string>; keywords?: string[]; }`                                                     | No       | Project metadata — auto-detected from `package.json` when not provided.                  |
